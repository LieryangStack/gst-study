...
static void
demux_pad_added_handler (GstElement *src, GstPad *new_pad, CustomData *data);

static void
multiqueue_pad_added_handler (GstElement *src, GstPad *new_pad, CustomData *data);
...
main (int argc, char *argv[]) {
  ...
  if (!data.pipeline || !data.source || !data.demux || data.multiqueue, \
      !data.vorbisdec || !data.audio_convert || !data.audio_resample || !data.audio_sink || \
      !data.video_vp8dec || ! data.video_convert || !data.video_sink) {
    g_printerr ("Not all elements could be created!.\n");
    return -1;
  }

  gst_bin_add_many (GST_BIN (data.pipeline), data.source, data.demux, data.multiqueue, \
                      data.vorbisdec, data.audio_convert, data.audio_resample, data.audio_sink, \
  ...
  if (!gst_element_link_many (data.vorbisdec, data.audio_convert, \
  ...
  if (!gst_element_link_many (data.video_vp8dec, data.video_convert, \
  ...
  g_signal_connect (data.demux, "pad-added", G_CALLBACK (demux_pad_added_handler), &data);
  g_signal_connect (data.multiqueue, "pad-added", G_CALLBACK (multiqueue_pad_added_handler), &data);
...
demux_pad_added_handler (GstElement *src, GstPad *new_pad, CustomData *data){
  ...
  g_print ("demux_pad_added_handler...\n");

  GstPadLinkReturn ret;
  GstPad *sink_pad = NULL;

  sink_pad = gst_element_get_request_pad  (data->multiqueue, "sink_%u");
  g_print ("Obtained request pad %s\n", gst_pad_get_name (sink_pad));

  ret = gst_pad_link (new_pad, sink_pad);
  if (GST_PAD_LINK_FAILED (ret)) {
    g_print ("Link failed.\n");
    gst_element_release_request_pad (src, sink_pad);
  } else {
...
multiqueue_pad_added_handler (GstElement *src, GstPad *new_pad, CustomData *data) {
  ...
  g_print ("multiqueue_pad_added_handler...\n");
  ...
  if (g_str_has_prefix (GST_PAD_NAME (new_pad), "sink")) {
    return;
  }
  static int i = 0;
  ...
  if (i++ == 0) {
    ...
    sink_pad = gst_element_get_static_pad (data->vorbisdec, "sink");
    /* Attempt the link */
    ret = gst_pad_link (new_pad, sink_pad);
    if (GST_PAD_LINK_FAILED (ret)) {
      g_print ("Type is '%s' but link failed.\n", new_pad_type);
