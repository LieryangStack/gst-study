...
/**
 * print_pad_capabilities(GstElement *element, gchar *pad_name) 通过元素和pad名称得到 GstCaps
...
/**
 * 打印每个结构里面的内容，例如video/x-bayer结构，会打印以下内容
...
print_caps(const GstCaps *caps, const gchar *pfx){
  ...
  for(i = 0; i < gst_caps_get_size(caps); i++){
    ...
    /* pfx = "      "就是空格字符串
     * gst_structure_get_name(structure) 就是 video/x-raw，video/x-bayer 等等
     */
 ...
 gst_bin_add_many (GST_BIN (pipeline), source, sink, NULL);
  ...
  do{
    msg = gst_bus_timed_pop_filtered(bus, GST_CLOCK_TIME_NONE,
        GST_MESSAGE_EOS | GST_MESSAGE_STATE_CHANGED | GST_MESSAGE_ERROR);
    if(msg != NULL){
      GError *err;
      gchar *debug_info;
