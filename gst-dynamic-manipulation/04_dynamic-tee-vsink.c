...
unlink_cb (GstPad * pad, GstPadProbeInfo * info, gpointer user_data)
{
  ...
  /* 因为添加监听的timeout_cb函数和unlink_cb函数不在同一线程，所以unlink_cb函数可能会立即被调用 */
  ...
  g_print ("GST_OBJECT_REFCOUNT_VALUE(sink->sink) = %d\n", GST_OBJECT_REFCOUNT_VALUE(sink->sink));
  g_print ("GST_OBJECT_REFCOUNT_VALUE(sink->conv) = %d\n", GST_OBJECT_REFCOUNT_VALUE(sink->conv));
  g_print ("GST_OBJECT_REFCOUNT_VALUE(sink->queue) = %d\n", GST_OBJECT_REFCOUNT_VALUE(sink->queue));
