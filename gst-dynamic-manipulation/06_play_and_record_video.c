...
create_encode_file_bin (CustomData * data) {
  ...
  gst_object_ref (data->file_queue);
  gst_object_ref (data->nvv4l2_h264enc);
  gst_object_ref (data->file_h264_parse);
  gst_object_ref (data->qtmux);
  gst_object_ref (data->file_sink);

  if (!data->file_queue || !data->nvv4l2_h264enc || !data->file_h264_parse || \
  ...
  switch (GST_MESSAGE_TYPE (msg)) {
    ...
    case GST_MESSAGE_APPLICATION :
      if (gst_message_has_name (msg, "PAUSED")) {
        ...
        gst_object_unref (data->file_queue);
        gst_object_unref (data->nvv4l2_h264enc);
        gst_object_unref (data->file_h264_parse);
        gst_object_unref (data->qtmux);
        gst_object_unref (data->file_sink);
