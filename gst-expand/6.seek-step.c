...
send_seek_event(CustomData *data){
  ...
  /* 创建seek事件,seek可以作用于pipeline中所有的Element */
  if(data->rate > 0){
    seek_event = 
        /* GST_SEEK_FLAG_FLUSH 跳转后会丢弃当前pipeline里面所有的数据 */
...
main(int argc, char *argv[]){
  ...
  data.pipeline = 
    gst_parse_launch
      ("playbin uri=file:///opt/nvidia/deepstream/deepstream-5.1/samples/streams/sample_720p.mp4",
      NULL);
  x_overlay = gst_element_factory_make("xvimagesink", "videosink");
  g_object_set (data.pipeline, "video-sink", x_overlay, NULL);
